console.log("JavaScript #2 HomeWork #16");

/*
 * #1
 * Создаём объект userObj, описывающий человека.
...
/*
 * #2
 * Для объекта из п.1 создаём метод fullName, который будет возвращать полное имя,
...
/*
 * #4
 * Создаём функцию evenFn(n), которая принимает параметром число – количество итераций цикла,
 * т.е. for 0..n. Функция возвращает массив, состоящий только из четных значений, генерируемых в цикле.
 * * Причем:
 * 0 не должен попадать в результирующий массив
 * цикл работает до n включительно
 * разрезаем только оператор for
 *
 * Например:
 ...
 * создаем функцию weekFn(n), которая принимает номер дня недели, а возвращает его название.
 ...
 * 7 → 'Воскресенье'
 *
 * В реализации функции обязательно используем операторы switch / case / default.
 */
...
/*
 * #6
 * создаём функцию ageClassification(n), которая будет в качестве параметра принимать любые числа
...
/*
 * #7
 * создаем функцию oddFn(n), которая принимает параметром число – количество итераций цикла.
...
/*
 * #8
 * создаем основную функцию mainFunc(a, b, func), которая принимает три параметра:
 * a – число
 * b - число
